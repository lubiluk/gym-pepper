...
class PepperArmVelPushEnv(gym.GoalEnv):
    ...
    def step(self, action):
        ...
        joint_p = np.array(self._robot.getAnglesPosition(CONTROLLABLE_JOINTS))
        vels = np.array(action) * 0.1

        self._robot.setAngles(CONTROLLABLE_JOINTS, list(joint_p + vels), [self._max_speed] * 6)
